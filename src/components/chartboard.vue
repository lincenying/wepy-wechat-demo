...
export default class Chartboard extends wepy.component {
    ...
    async sendMessage (msg) {
        const id = this.to
        const msgObj = await api.sendMsg(id, msg)
        ...
        const randomReply = await api.getRandomReply(id)
        this.replyMessage(randomReply)
    }

    async replyMessage (msg) {
        const id = this.to
        const contact = global.getContact(id)
        const msgObj = await api.replyMsg(id, msg)
